...
export default function ChapterWorkspacePage() {
  ...
  return <main className="workspace"><header><Link href="/study">← Study</Link><div><span>LAKSHYA • CHAPTER WORKSPACE</span><h1>{chapter}</h1><p>{subject} · AI-powered learning space</p></div><div className="topActions"><button onClick={exportWorkspace}>↗ Export</button><button className="primary" disabled={saving} onClick={() => void saveAll()}>{saving ? "Saving…" : "Save Workspace"}</button></div></header>
