...
type Session = { minutes?: number; createdAt?: number };
type Attempt = { subject?: string; correct?: boolean; createdAt?: number };
...
function getStreak(dates: Set<string>) {
  const cursor = new Date(); cursor.setHours(0, 0, 0, 0); let streak = 0;
...
export default function AnalyticsPage() {
  ...
  useEffect(() => subscribeToAuth(async (user) => {
    ...
    if (!user) { setSessions([]); setAttempts([]); setTasks([]); setLoading(false); return; }
    setLoading(true);
    try {
      const base = `users/${user.uid}`;
      const [sessionSnap, attemptSnap, taskSnap] = await Promise.all([
        get(ref(realtimeDb, `${base}/studySessions`)),
        get(ref(realtimeDb, `${base}/practiceAttempts`)),
        get(ref(realtimeDb, `${base}/plannerTasks`)),
      ]);
      setSessions(records<Session>(sessionSnap.val()));
      setAttempts(records<Attempt>(attemptSnap.val()));
      setTasks(records<Task>(taskSnap.val()));
    } catch (error) { console.warn("Lakshya analytics load skipped:", error); }
    finally { setLoading(false); }
  }), []);

  const stats = useMemo(() => {
    const now = new Date(); const weekStart = new Date(now); weekStart.setHours(0, 0, 0, 0); weekStart.setDate(weekStart.getDate() - 6);
    const weeklySessions = sessions.filter(s => Number(s.createdAt) >= weekStart.getTime());
    const minutes = weeklySessions.reduce((sum, s) => sum + Number(s.minutes || 0), 0);
    const correct = attempts.filter(a => a.correct).length;
    const accuracy = attempts.length ? Math.round((correct / attempts.length) * 100) : 0;
    const activityDates = new Set<string>();
    sessions.forEach(s => { if (s.createdAt) activityDates.add(dateKey(new Date(s.createdAt))); });
    attempts.forEach(a => { if (a.createdAt) activityDates.add(dateKey(new Date(a.createdAt))); });
