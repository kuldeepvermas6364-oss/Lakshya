...
function PremiumNavIcon({ type }: { type: "home" | "study" | "practice" | "ai" | "community" }) {
  ...
  if (type === "ai") return <svg {...common}><path d="m12 2 1.55 5.45L19 9l-5.45 1.55L12 16l-1.55-5.45L5 9l5.45-1.55z"/><path d="m19 14 .7 2.3L22 17l-2.3.7L19 20l-.7-2.3L16 17l2.3-.7z"/></svg>;
...
export default function AppFrame({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-frame">
    ...
    <nav className="frame-mobile-nav" aria-label="Mobile navigation">
      {[["home","Home","/"],["study","Study","/study"],["practice","Practice","/practice"],["ai","AI","/ai"],["community","Community","/community"]].map(([type,label,href]) => <Link key={href} href={href} className={active(href) ? "active" : ""}><b><PremiumNavIcon type={type as "home" | "study" | "practice" | "ai" | "community"} /></b><small>{label}</small></Link>)}
    </nav>
    <LakshyaAI />
  </div>;
}
