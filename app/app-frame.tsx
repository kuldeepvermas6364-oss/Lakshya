  ...
  return <div className="app-frame">
    ...
    <div className="frame-content">
      <header className="frame-topbar">
        <Link href="/" className="frame-mobile-brand"><img src="/lakshya-mark.svg" alt="" /><b>Lakshya</b></Link>
        <Link href="/search" className="frame-search" aria-label="Search Lakshya">⌕ <span>Search chapters, notes, questions...</span></Link>
        <div className="frame-actions"><Link href="/premium" className="frame-premium-top">✦ Premium</Link><Link href="/notifications" className="frame-icon" aria-label="Notifications">♢</Link><Link href="/profile" className="frame-avatar" aria-label="Profile">K</Link></div>
      </header>
      {children}
    </div>
    <nav className="frame-mobile-nav" aria-label="Mobile navigation">
      {[["⌂","Home","/"],["◫","Study","/study"],["✓","Practice","/practice"],["▣","Exams","/exams"],["✦","AI","/ai"]].map(([icon,label,href]) => {
        const isAi = href === "/ai";
        const className = active(href) ? (isAi ? "active premium-tab" : "active") : (isAi ? "premium-tab" : "");
        return <Link key={href} href={href} className={className}><b>{icon}</b><small>{label}</small></Link>;
      })}
    </nav>
    <LakshyaAI />
  </div>;
