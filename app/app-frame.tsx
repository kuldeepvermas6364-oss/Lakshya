...
function PremiumNavIcon({type}:{type:"home"|"study"|"practice"|"ai"|"multi-ai"|"community"|"career"}) {
 ...
 if(type==="multi-ai") return <svg {...common}><circle cx="7" cy="7" r="2.5"/><circle cx="17" cy="7" r="2.5"/><circle cx="7" cy="17" r="2.5"/><circle cx="17" cy="17" r="2.5"/><path d="M9 7h6M7 9v6M17 9v6M9 17h6"/></svg>;
...
export default function AppFrame({children}:{children:React.ReactNode}){
 ...
 const nav=[["✦","Lakshya AI","/ai"],["◈","Multi-Model AI","/multi-ai"],...primary,...extra,...social];
 return <div className="app-frame">
  ...
  <div className="frame-content"><header className="frame-topbar"><Link href="/" className="frame-mobile-brand"><img src="/lakshya-mark.svg" alt=""/><b>Lakshya</b></Link><div className="frame-search">⌕ <span>Search chapters, notes, questions...</span></div><div className="frame-actions"><Link href="/multi-ai" className="frame-premium-top">◈ Compare AI</Link><Link href="/ai" className="frame-premium-top">✦ Ask AI</Link><Link href="/premium" className="frame-premium-top">✦ Premium</Link><Link href="/notifications" className="frame-icon" aria-label="Notifications">♢</Link><Link href="/profile" className="frame-avatar">K</Link></div></header>{children}</div>
  <nav className="frame-mobile-nav" aria-label="Mobile navigation">{[["home","Home","/"],["study","Study","/study"],["practice","Practice","/practice"],["ai","AI","/ai"],["multi-ai","Multi AI","/multi-ai"],["community","Community","/community"]].map(([type,label,href])=><Link key={href} href={href} className={active(href)?"active":""}><b><PremiumNavIcon type={type as "home"|"study"|"practice"|"ai"|"multi-ai"|"community"|"career"}/></b><small>{label}</small></Link>)}</nav>
  <LakshyaAI/>
 </div>;
}
