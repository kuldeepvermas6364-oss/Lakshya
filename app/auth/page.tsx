...
export default function AuthPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "register") { await registerUser(name, email, password); router.replace("/onboarding"); }
      else { await loginUser(email, password); router.replace("/"); }
    } catch (err) { setError(friendlyAuthError(err)); } finally { setBusy(false); }
  }

  async function handleReset() {
    setError(""); setNotice(""); if (!email.trim()) return setError("Enter your email first."); setBusy(true);
  ...
  return <main className="auth-page-premium"><div className="auth-orb orb-one"/><div className="auth-orb orb-two"/>
    ...
    <section className="auth-card-premium" aria-labelledby="auth-title"><div className="auth-card-top"><span>{mode === "login" ? "WELCOME BACK" : "GET STARTED"}</span><div className="secure-badge">⌁ Secure</div></div><h2 id="auth-title">{mode === "login" ? "Sign in to Lakshya" : "Create your account"}</h2><p className="auth-subtitle">{mode === "login" ? "Pick up exactly where you left off." : "Your personalized study space starts here."}</p>
      <form onSubmit={handleSubmit} className="auth-form-premium">{mode === "register" && <label><span>Name</span><input value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" autoComplete="name" required minLength={3} maxLength={30}/></label>}<label><span>Email</span><input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="you@example.com" autoComplete="email" required maxLength={254}/></label><label><span>Password</span><input value={password} onChange={e=>setPassword(e.target.value)} type="password" placeholder="At least 6 characters" autoComplete={mode === "login" ? "current-password" : "new-password"} required minLength={6} maxLength={128}/></label>{error&&<p className="auth-error-premium" role="alert">{error}</p>}{notice&&<p role="status">{notice}</p>}<button className="auth-submit-premium" disabled={busy} type="submit"><span>{busy?"Working…":mode === "login"?"Enter Lakshya":"Create my Lakshya"}</span><b>→</b></button></form>
      {mode === "login"&&<button className="auth-switch-premium" disabled={busy} onClick={handleReset}>Forgot password?</button>}<button className="auth-switch-premium" onClick={()=>{setMode(mode === "login"?"register":"login");setError("");setNotice("")}}>{mode === "login"?"New here? Create an account":"Already have an account? Sign in"}</button><p className="auth-note-premium">Your account data stays protected. Never share your password or OTP.</p>
    </section></main>;
}
