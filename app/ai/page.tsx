...
export default function AIPage() {
  ...
  async function ask(text = input) {
    ...
    try {
      const res = await fetch("/api/ai/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: value, context }) });
  ...
  return <main className="page lakshya-ai-page">
    ...
    <section className="panel ai-workspace" style={{ marginTop: 14, padding: 0, overflow: "hidden" }}>
      ...
      <div className="ai-conversation" aria-live="polite">
        {messages.length === 0 && !loading ? <div className="ai-empty"><div className="ai-empty-icon">✦</div><h3>Ready when you are.</h3><p>Ask a question, paste a concept you do not understand, or choose a prompt above.</p></div> : messages.map((message) => <article key={message.id} className={`ai-message ${message.role}`}><span className="ai-message-label">{message.role === "user" ? "YOU" : "LAKSHYA AI"}</span>{message.role === "ai" ? <RichAIResponse text={message.text} /> : <p>{message.text}</p>}</article>)}
      ...
      <form onSubmit={submit} className="ai-composer"><textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Lakshya AI about a topic, question, revision or study plan…" rows={2} maxLength={4000} disabled={loading} /><div className="ai-composer-foot"><span>{input.length}/4000 · {subject}</span><button className="primary" disabled={!input.trim() || loading}>{loading ? "Thinking…" : "Ask AI →"}</button></div></form>
    </section>
