...
type Post = { name: string; initials: string; time: string; tag: string; text: string; replies: number; likes: number; media?: MediaMetadata[] };
...
export default function CommunityPage() {
  ...
  function publish() {
    ...
    setPostsState([{ name: "You", initials: "Y", time: "Just now", tag: "Discussion", text, replies: 0, likes: 0, media: composerMedia }, ...postsState]);
    setComposer("");
    setComposerMedia([]);
  }
  ...
  return <main className="community-page">
    ...
    <div className="community-layout">
      <section>
        ...
        {postsState.map((post, i) => <article className="post" key={`${post.name}-${i}`}><div className="post-head"><div className="avatar-small">{post.initials}</div><div><b>{post.name}</b><small>{post.time} · Public</small></div><button>•••</button></div><span className="post-tag">{post.tag}</span>{post.text && <p>{post.text}</p>}{post.media?.length ? <MediaGallery media={post.media} /> : null}<div className="post-actions"><button>♡ {post.likes}</button><button>◌ {post.replies} replies</button><button>↗ Share</button></div></article>)}
      </section>
