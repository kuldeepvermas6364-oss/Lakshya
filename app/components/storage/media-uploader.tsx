...
export function MediaUploader({ category = "posts", multiple = true, accept = "image/*,video/*,.pdf,.txt", maxFiles = 8, onComplete }: Props) {
  ...
  return <div className="storage-uploader">
    ...
    <style jsx>{`.storage-uploader{width:100%}.storage-drop{width:100%;border:1px dashed #cfd5db;background:linear-gradient(180deg,#fff,#f8fafb);border-radius:14px;padding:20px 14px;display:flex;flex-direction:column;align-items:center;gap:5px;cursor:pointer;transition:transform .2s ease,border-color .2s ease,background .2s ease}.storage-drop:hover,.storage-drop.is-dragging{transform:translateY(-1px);border-color:#87929d;background:#f4f7f9}.storage-icon{width:34px;height:34px;border-radius:10px;background:#18212b;color:#fff;display:grid;place-items:center;font-weight:900;font-size:17px;margin-bottom:3px}.storage-drop strong{font-size:12px}.storage-drop small{font-size:9px;color:#87919b;text-align:center}.storage-list{display:grid;gap:7px;margin-top:9px}.storage-item{display:flex;align-items:center;gap:9px;padding:9px;border:1px solid #e7eaed;background:#fff;border-radius:11px}.storage-file-icon{width:34px;height:34px;border-radius:9px;background:#eef1f3;display:grid;place-items:center;font-size:7px;font-weight:900}.storage-info{min-width:0;flex:1}.storage-info strong,.storage-info small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.storage-info strong{font-size:10px}.storage-info small{font-size:8px;color:#8a949e;margin-top:3px}.storage-progress{height:3px;background:#e9edf0;border-radius:99px;overflow:hidden;margin-top:6px}.storage-progress span{display:block;height:100%;background:#18212b;border-radius:99px;transition:width .2s ease}.storage-controls{display:flex;align-items:center;gap:5px}.storage-controls button{border:0;background:none;font-size:9px;font-weight:800;color:#68737d;cursor:pointer;padding:5px}.storage-controls button:last-child{font-size:17px;padding:2px 5px}`}</style>
  </div>;
}
