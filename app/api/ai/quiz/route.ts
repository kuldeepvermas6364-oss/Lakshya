...
export async function POST(request: Request) {
  try {
    ...
    const raw = (await generateGeminiContent(prompt, "You are Lakshya AI, an academic quiz generator for Class 12 PCM students. Generate accurate, syllabus-aligned questions. Return ONLY valid JSON, no markdown.")).trim().replace(/^```json\s*/i, "").replace(/```$/i, "").trim();
