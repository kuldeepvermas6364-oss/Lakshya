...
export default function PollinationsAIPage() {
  ...
  const [error, setError] = useState("");\n  const [featureType, setFeatureType] = useState<FeatureType>("chat");
  ...
  async function send(text = input) {
    ...
    try {
      ...
      setMessages([...next, { id: Date.now() + 1, role: "assistant", content: String(data.text || ""), image: typeof data.image === "string" ? data.image : undefined }]);
    } catch (e) {
  ...
  return (
    <main className="polli-page">
      ...
      <section className="polli-card">
        <div className="polli-modes">\n          {([["chat","Chat"],["code","Code Expert"],["image","Image Creator"]] as const).map(([value,label]) => (\n            <button key={value} type="button" className={featureType === value ? "active" : ""} onClick={() => setFeatureType(value)} disabled={loading}>{label}</button>\n          ))}\n        </div>\n\n        <div className="polli-toolbar">
